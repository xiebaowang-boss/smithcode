...
- **11 个内置工具**：读 / 写 / 精确编辑文件（读文件带行号、支持分段读取；写 / 编辑强制先读后写，编辑支持 replace_all）、apply_patch 批量原子改文件、列目录、glob 文件名搜索（按修改时间排序）、grep 内容搜索（支持忽略大小写、上下文行、只列文件/计数模式）、webfetch 网页抓取、执行 shell 命令（默认 60 秒超时，可延长至 300 秒）、任务中途向用户提问（ask_user）、维护任务拆分清单（todo_write）
