...
- **自动持久化**：每条非 system 消息实时追加到本地 JSONL 转录（`~/.smithcode/projects/<项目>/sessions/<会话 id>.jsonl`），崩溃 / 关窗也能恢复；**工具执行前与每轮结束各做一次 fsync 屏障**——已记录的工具调用不会「副作用已发生、调用却没落盘」，每轮工作也不会因断电整段丢失
...
- **崩溃修复**：工具结果落盘前进程被杀留下的悬空 `tool_calls`，恢复时自动补占位或截断，保证历史合法（占位声明「结果未知」并提示先核实，不会诱使模型盲目重试写操作）
- **模型留痕**：每轮实际使用的模型与思考强度写入转录（`/model` 中途切换也留痕），恢复时提示「该会话上次使用模型 X」，不悄悄改当前全局模型
