...
- **「输出中断」不带原因，无法排障**：`stream_error` 此前只显示「· 输出中断 / ⏹ 输出中断（内容不完整…）」，不说断在哪类故障上——用户看到「发一句话就输出中断」却没有任何线索（是读超时？限流？对端掐断？）。现失败原因全程带出：`RunResult.reason` 存 `分类: 原始信息`（如 `读取超时: The read operation timed out`），TUI 页脚显示「· 输出中断（读取超时: …）」，控制台同样打印，回写历史的中断说明也带上原因。**读超时额外给可操作提示**并澄清语义：`[limits].llm_timeout`（默认 120s）是**空闲超时**——两次收到数据之间静默超过它就断开，不是整个请求的总时长上限；推理型模型首字延迟可能很长，提示用户调大该值。已补回归测试（原因进历史与 `result.reason`、页脚带原因、超时提示只在超时类故障出现）。
- **流中断就报废整轮 → 片段重试（对齐上游）：重试策略收敛为单一权威 + 重试进度上屏**：`client.chat_stream` 的重试循环里有一行 `if ... or printed: raise`——**"正文已输出过就不再重试"**。模型响应流断在正文中间（读完超时 120s 静默 / 对端掐断连接）时正文早已实时上屏，于是这一行直接放弃整轮：一次瞬时故障报废 40 分钟成果（用户侧：总结刚打一行就 `✗ ReadTimeout`，会话停止）。上游三家（opencode `Effect.retry` + `SessionRetry.policy`、Codex stream retry/fallback、Claude Code）都是**整请求重发**，从不因"已输出正文"放弃。现：① 新增 `llm/retry.py` 作为**重试策略与状态机的唯一权威**（此前散在三处且语义各异：client 内联循环、`_chat_with_recovery` 的溢出恢复、自动标题的计数冷却）——`RetryPolicy`（分类 / 预算 / 退避，2s 起步 ×2、25% 抖动、上限 30s、服务端 `Retry-After` 优先）、`RetryState`（进行态 + 倒计时文案，TUI 与终端共用同一份）、`RetryRunner` / `wait`（可被 Esc 打断的分段退避）、`retryable` / `classify`（429 / 5xx / 传输层可重试；4xx、鉴权、上下文溢出、取消一律不重试）；② 删除 `printed` 判据，正文已上屏照常重试，且**每次尝试的正文都累积进同一条 assistant 消息**（对齐 opencode 在同一条消息里累积多个 text part：屏幕上出现过两段，历史里就都有两段，模型下一轮能看到完整上下文）；③ 重试进度经新增 `Renderer.retry_started` / `retry_finished` 上报，**TUI 显示在输入框上方那一行**（`⠹ Working… 12s · 正在重试 2/3 · 8s 后`，倒计时由 10Hz 转轮自刷，不额外推事件），已上屏的那段正文追加「（已中断，正在重试…）」标记以免与重试后的正文无法区分；事件带 owner，前台任务与后台标题（共用 client）的重试互不误清；其他后端降级为一行 `↻ …` 提示。预算耗尽才落到 `stream_error` 收尾（部分正文入库 + `STREAM_INTERRUPTED_CONTEXT`）。可重试判定补两层对齐上游：① **错误文本兜底**（对齐 opencode 的 `RETRYABLE_MESSAGE_PATTERNS`）——有些服务商把瞬时故障包成普通异常（`Exception("stream timeout")`），类型与状态码都判不出来，现按文案兜底（限流 / 过载 / 连接层 / 流被掐断 / 超时，含两种语序的超时表述），且刻意放在永久错误之后——4xx 响应体里恰好含 `timeout` 不会被误判成可重试；响应体（`body`）里的文案也参与判定。② **`Retry-After` 不再被本地退避上限砍掉**（对齐 opencode 的`RETRY_MAX_DELAY_NO_HEADERS` 语义）——服务端明确要求等 120s 时不再缩到 30s 提前重试（只会再撞一次限流），改为只受宽上限 300s 约束；错误文案里的 `retry_after: N` 同样照办。已补回归测试：`test_llm_retry.py`（分类 / 预算 / 退避 / `Retry-After` 与宽上限 / 文本兜底及其边界 / 文案倒计时 / 取消打断退避）、`test_llm.py`（已输出正文仍重试、只有最后一次尝试的 message/usage 生效、不可重试错误一次即抛、成功也发 `retry_finished`）、`test_agent.py`（两段正文同入一条消息）、`test_tui.py`（重试进度文案与倒计时、中断标记、owner 不误清）。
- **响应流中途断开后：部分正文丢失 + 下一轮内容黏进上一轮的块**：模型响应流断在正文中间（读完超时 `ReadTimeout` / 对端掐断连接）时，正文是边收边实时上屏的，但 ① `_chat()` 只在流正常跑完后才组装 assistant 消息，异常直接把已收到的正文丢掉——屏幕上看到的总结没进会话历史，下一轮模型看不见自己的输出，于是**从头重做一遍、再复述一遍**（用户侧表现为"问它完成没有，它把上一轮总结又输出了一遍"）；② `r.stream_done()` 只在 `for` 之后调用、无 `try/finally`，异常打断后 TUI 的正文块永不收口（`ChatView._kind` 停在 `content`、`_body` 仍是旧块），下一轮的增量被 `append_stream` 直接追加进同一个块——两轮内容黏在一起、按新宽度整块重排，看起来像错位叠加。现：`_chat()` 用 `try/finally` 保证 `stream_done()` 一定执行（收口幂等），重试预算用尽后把 `StreamInterrupted(original, partial)` 上抛，`run()` 捕获后把**已上屏的部分正文按 assistant 消息入库**并追一条中断说明（`STREAM_INTERRUPTED_CONTEXT`，不触发新请求），返回新状态 `stream_error`（`text` 即部分正文）；宿主据此提示——TUI 页脚补「· 输出中断」、控制台打 `STREAM_INTERRUPTED_NOTE`。首块之前就断开（部分正文为空）不留空 assistant 消息；溢出恢复路径再断开不会重复记账；上下文溢出恢复语义不变（首块前断且判定为溢出时仍压缩重试）。已补回归测试（部分正文入库且只落一次、无空 assistant、TUI 两轮各占一块）。
- **grep 符号链接越界读**：`_iter_files` 的 walk 分支对 lexical 路径直接 `stat/read`，工作区内的 `link -> /etc/passwd` 会被读出（glob 有 `resolve` 校验、grep 单文件路径经 `_roots` 规范化，只有该分支漏了）。现逐文件 `resolve()` 后用任一读根重判，跨根链接按目标实际所在根展示路径；符号链接目录不深入（防循环与越界遍历）。同时 `glob` 的根校验从"起始根"放宽为"任一读根"（workspace 链接到附加授权目录内文件不再误丢），`.env` 保护扩大到 `.env.*` 变体，`list_dir` 对不存在/非目录/无权限返回中文友好错误而非抛异常。已补回归测试。
- **检索与读取的超长行静默截断**：`grep` / `read_file` 截断超长行后无任何标记，Agent 把半行复制成 `edit_file` 的 `old_string` 必然「未找到」。现统一截断并追加 `…` 后缀，工具描述同步告知"截断处不可逐字复制"。已补回归测试。
- **glob 扫描预算熔断**：`grep` 有 5000 文件 / 50MB 熔断，`glob` 无匹配时却走完全树。现 `glob` 复用同一套 `MAX_SCAN_FILES`（按遍历项计），超限即停并提示收窄。已补回归测试。
- **list_dir 符号链接目录跟随**：`list_dir` 用默认 `is_dir()` / `stat()`（跟随软链），指向仓外的目录软链会泄漏目标的目录性 / 大小 / mtime。现改 `follow_symlinks=False`（lstat 语义，软链按链接本身展示），与 `glob` / `grep` 的遍历口径一致。已补回归测试。
- **edit_file 多匹配崩溃**：`_match_linenos` 早停循环凑满 5 个后漏 `return`（隐式 `None`），`edit_file` 对 `None[:5]` 直接抛 `TypeError`——≥5 处匹配且未开 `replace_all` 时崩溃而非中文报错。现补尾部 `return linenos`。已补回归测试。
- **删除本地检索死代码 `iter_files`**：`grep` / `glob` 均已直调 `walk_entries`，`_shared_local.iter_files` 无任何引用。现删除，无行为变化。
...
- **TUI「已探索」汇总头按整组结算**：状态此前只看「组内是否还有没出结果的子工具」，组内单个工具先收工、后续同组工具还没开始时，中间空档会闪变成「▸ ⚙ 已探索」（本地工具毫秒级结束时尤其明显）。现改为**封口 + 收齐才结算**：组创建起即「探索中」，单个工具收工不再翻转；遇到非上下文工具 / 正文 / 思考流 / 回合结束封口（不会再有新工具加入本组）并且全部子工具出结果后，才切「▸ ⚙ 已探索 · …」并停掉转轮；轮次结束兜底（`drain_pending`）同时封口。已补回归测试（收齐未封口仍「探索中」、同组工具时间空档不闪变、封口 + 收齐才「已探索」）。

- **TUI 对话区贴底后仍不跟随新输出**：滚动跟随此前是"贴底就 `scroll_end()`"，而 `scroll_end` 的目标行号取自发出那一刻的滚动区高度——流式正文按帧节流重排（落在 16ms 窗口内的 chunk 不触发布局，目标行号因此算在旧高度上）、输入区高度可变（运行动画出现 / 命令菜单弹出 / 输入框增高都会让聊天区变矮）都会让实际落点比预期差出 1 行以上；一旦超过 1 行容差，"是否贴底"的判断此后恒为假，跟随**永久失效**（需手动滚一下才恢复）。现改为**贴底意图 + 布局期贴底**：意图（`ChatView._stick`）只随真实滚动位置变化——`watch_scroll_y` 里用户滚走即关、滚回底部即开；贴底动作放在 `_size_updated`（合成器排版之后调用，任何尺寸变化都会触发）里按真实内容高度重算。被节流推迟到后续布局的流式正文、聊天区高度变化（输入框 / 运行动画 / 命令菜单）、工具结果与变更预览这类"就地把块改高"的增长因此全部自动覆盖，不再需要在每个内容增长点各自判断（那里拿到的 `max_scroll_y` 常是增长前的旧值，容易漏判）。**不用 Textual 自带的 `anchor()`**：它算的贴底位置是「内容底 − 容器高」，内容不足一屏时为负，而合成器经不校验的 `set_reactive` 写入（文档明示绕过校验器与 watcher），负偏移会把整块内容推到视口下方——表现为欢迎 Logo 落在对话区底部、第一轮一有输出整屏内容跟着上移。改为自行赋值以走 `scroll_y` 校验器（天然夹在 `[0, max_scroll_y]`，不足一屏时即 0，贴底即顶部），并同步 `scroll_target_y`（滚轮 / 翻页是相对滚动，只改 `scroll_y` 会让贴底后第一下上滚直接跳到顶部）。

- **工具行被多行命令撑成多行**：`run_command` 的 `describe` 会把命令原文拼进摘要，而命令本身可以带换行（heredoc `python - <<'PY'` 等），且截断按**字符**计数——换行原样进入摘要后，折叠状态下这一行被渲染成 2~3 行，看起来像"正文展开了"，实际上正文一行都没显示（`block` 形态未展开时根本不渲染正文，只有摘要行）。现摘要生成后**先压平空白再按 80 字符截断**（`agent.py`，一处覆盖 TUI / REPL / 单次模式，pending 期的「执行中」行同时受益；放进截断之前可保证字符上限全部花在可见内容上，权限确认框拿到的 `content` 也随之已是单行）。另有第二类多行来自**列宽**——80 字符摘要放在窄聊天区里会被终端折行，故 TUI 侧 `.tool-header` 补 `text-wrap: nowrap` + `text-overflow: ellipsis`，超列宽按实际宽度出省略号。两者配合后工具行恒占一行；完整命令仍可在权限确认框看到（那里由 `_clip` 自行压平）。已补回归测试。
- **Shift+Tab 让侧边栏标题"跟着变化"**：`action_cycle_permission_mode` 每次都调全量 `ui_status()`，后者无条件重写侧边栏标题等控件——而 `Static.update` 无论内容变没变都触发整栏重排，空闲时按一次标题就闪一下，看起来像被改掉了（实际文本没变）。现 Shift+Tab 只刷新底栏 `#composer-mode` 一段；另给 `Sidebar.update_title` 加同内容去重——标题真没变时一次 `update` 都不发，顺带兜住其他无关刷新。已补回归测试（切换不碰标题控件、同内容跳过 `update`）。
- **首轮中断后自动标题"后续不再生成"**：两处叠加。① 标题 payload 取"第一个 user + 其后第一个 assistant"，而中断回写（`INTERRUPTED_CONTEXT`）本身就是一条 user 消息——首轮被中断后它恰好落在"第二个 user"的位置，`build_title_request` 就此截断：首轮的部分输出被丢掉、中断残留被当成轮次边界，标题要么被污染、要么和后面的轮次搅在一起。现只取首个**真实用户轮**：中断回写 / 流中断回写 / 压缩提示 / 技能回找引导 / 迭代上限收尾五类记账消息一律跳过，轮次边界仍是真实用户轮。② 标题 3 次失败到顶后永久沉默（"已停止重试"即终点），而失败常与当前模型相关（不按 JSON 输出、拒标题请求）。现到顶后进入 5 轮冷却再自动探一次；`/model` 切换模型立即重置计数（下一轮正常结束即再试）。已补回归测试（记账消息跳过、前缀一致性守卫、冷却再探、切换重置、中断后跨轮补生成）。
- **自动标题到顶提示补全**：到顶文案此前只说"可用 /rename 手动命名"，没说还会自动再试——用户以为功能永久关闭。现一并告知"5 轮后会自动再试一次，切换模型会立即重试"。
...
- **移除 `/skill` 命令（技能名即命令）**：每个技能名本身就是命令（`/技能名 [任务]`，同时并入 `/` 输入补全与 `/skills` 选择框），`/skill <名称>` 只是重复入口，现已删除——`/help` 与补全菜单不再出现该命令；`/skills` 选择框选中后改为按技能名直达分发（`CommandSelect.command` 留空即按 `/<value>` 分发，REPL 的非交互提示同步调整）。与内置命令重名的技能不进命令面（分发时内置命令优先）：此前尚可经 `/skill <名称>` 加载，现在只能由模型用 `use_skill` 加载，`/skills list` 仍会列出它们。
