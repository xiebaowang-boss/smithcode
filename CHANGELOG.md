...
- **响应流中途断开后：部分正文丢失 + 下一轮内容黏进上一轮的块**：模型响应流断在正文中间（读完超时 `ReadTimeout` / 对端掐断连接）时，正文是边收边实时上屏的，但 ① `_chat()` 只在流正常跑完后才组装 assistant 消息，异常直接把已收到的正文丢掉——屏幕上看到的总结没进会话历史，下一轮模型看不见自己的输出，于是**从头重做一遍、再复述一遍**（用户侧表现为"问它完成没有，它把上一轮总结又输出了一遍"）；② `r.stream_done()` 只在 `for` 之后调用、无 `try/finally`，异常打断后 TUI 的正文块永不收口（`ChatView._kind` 停在 `content`、`_body` 仍是旧块），下一轮的增量被 `append_stream` 直接追加进同一个块——两轮内容黏在一起、按新宽度整块重排，看起来像错位叠加。现：`_chat()` 用 `try/finally` 保证 `stream_done()` 一定执行（收口幂等），流中异常包成 `StreamInterrupted(original, partial)` 上抛，`run()` 捕获后把**已上屏的部分正文按 assistant 消息入库**并追一条中断说明（`STREAM_INTERRUPTED_CONTEXT`，不触发新请求），返回新状态 `stream_error`（`text` 即部分正文）；宿主据此提示——TUI 页脚补「· 输出中断」、控制台打 `STREAM_INTERRUPTED_NOTE`，用户一眼能看出这是残缺输出、可直接追问让模型接着写。首块之前就断开（部分正文为空）不留空 assistant 消息；溢出恢复路径再断开不会重复记账；上下文溢出恢复语义不变（首块前断且判定为溢出时仍压缩重试）。已补回归测试（部分正文入库且只落一次、无空 assistant、TUI 两轮各占一块）。
...
- **工具行被多行命令撑成多行**：`run_command` 的 `describe` 会把命令原文拼进摘要，而命令本身可以带换行（heredoc `python - <<'PY'` 等），且截断按**字符**计数——换行原样进入摘要后，折叠状态下这一行被渲染成 2~3 行，看起来像"正文展开了"，实际上正文一行都没显示（`block` 形态的折叠只作用于结果体）。现摘要生成后**先压平空白再按 80 字符截断**（`agent.py`，一处覆盖 TUI / REPL / 单次模式，pending 期的「执行中」行同时受益；放进截断之前可保证字符上限全部花在可见内容上，权限确认框拿到的 `content` 也随之已是单行）。另有第二类多行来自**列宽**——80 字符摘要放在窄聊天区里会被终端折行，故 TUI 侧 `.tool-header` 补 `text-wrap: nowrap` + `text-overflow: ellipsis`，超列宽按实际宽度出省略号。两者配合后工具行恒占一行；完整命令仍可在权限确认框看到（那里由 `_clip` 自行压平）。已补回归测试。
