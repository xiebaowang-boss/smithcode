...
- 提交信息即变更记录：按 `类型(范围): 中文简述` 写（如 `fix(tui): …`），发布时按标签区间从 git 历史整理
...
| `utils/http.py` | 网络工具的 HTTP 客户端工厂（`client()` / `read_limited()` / `ssl_context()`）：统一代理语义（`normalize_proxy_env` + `trust_env`，含 socks5）并屏蔽 ALPN（历史遗留的反爬指纹对策，源自已移除的 DuckDuckGo 后端，保留无害）；webfetch / websearch 共用 |
