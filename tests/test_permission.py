...
from smithcode.event import reset as reset_bus
from smithcode.event.asks import AskAnswer, AskPort
...
@pytest.fixture(autouse=True)
def _ask_port():
    """权限检查现在经询问端口提问：给每个用例挂一个（用完复位）。

    前端沿用未挂载时的**终端兜底**（读 stdin），所以既有 `builtins.input` 的
    打桩照旧生效；需要断言"问了什么"的用例用 `capture` 夹具覆盖前端。
    """
...
def check(perm, *args, **kwargs):
    """同步调用异步权限检查（`check()` 现在是协程：提问要 await 前端作答）。"""
    return asyncio.run(perm.check(*args, **kwargs))


def test_safe_tools_pass_without_asking(make_perm, monkeypatch):
    refuse_input(monkeypatch)
    assert asyncio.run(make_perm().check("read_file", {"path": "a.txt"})) is True
    assert asyncio.run(make_perm().check("list_dir", {})) is True


def test_webfetch_allowed_by_default(make_perm, monkeypatch):
    """webfetch 默认放行：抓取公开网页不再弹确认。"""
    refuse_input(monkeypatch)
    assert asyncio.run(make_perm().check("webfetch", {"url": "https://example.com"})) is True


def test_webfetch_user_rule_can_tighten(make_perm, monkeypatch):
    """用户规则命中优先于内置默认：可把 webfetch 收紧回 ask。"""
    monkeypatch.setattr("builtins.input", lambda _: "n")
    assert asyncio.run(make_perm({"webfetch": "ask"}).check("webfetch", {"url": "https://example.com"})) is False


def test_ask_denied_by_user(make_perm, monkeypatch):
    monkeypatch.setattr("builtins.input", lambda _: "n")
    assert asyncio.run(make_perm().check("write_file", {"path": "a.txt"})) is False


def test_ask_approved_once_does_not_remember(make_perm, monkeypatch):
    monkeypatch.setattr("builtins.input", lambda _: "y")
    perm = make_perm()
    assert asyncio.run(perm.check("write_file", {"path": "a.txt"})) is True
    assert perm.session_rules == []


@pytest.fixture
def capture():
    """捕获权限提示的两个通道：询问请求的内容 + Notice 事件。

    权限信息经询问端口的 `AskRequest` 传给前端（标题 / 选项 / 摘要 / 小字都在
    里面成形），提示文本走事件总线——所以两样都收。`calls` 保持改造前的元组
    形状 `(title, valid, detail, descriptions, content)`，断言不必重写。
    """
    ...
    class _Capture:
        ...
        async def ask(self, request):
            ...
            self.calls.append((
                ...
                tuple(request.detail),
                dict(payload.get("descriptions") or {}),
                payload.get("content"),
            ))
            return AskAnswer(outcome="answered", value="n")
    ...
    bus_token = activate_bus(bus)
    asker_token = frontend_mod.activate(cap)
    yield cap
    frontend_mod.reset(asker_token)
    reset_bus(bus_token)
...
def test_ask_renders_options_in_confirm_not_chat(make_perm, monkeypatch, capture):
    ...
    assert asyncio.run(perm.check("write_file", {"path": "a.txt"}, content="write a.txt")) is False
    assert cap.infos == []
...
def test_ask_truncates_long_content_and_option_descriptions(make_perm, monkeypatch, capture):
    ...
    assert asyncio.run(perm.check("run_command", {"command": command},
...
def test_ask_clips_content_to_single_line(make_perm, monkeypatch, capture):
    ...
    assert asyncio.run(perm.check("write_file", {"path": "a.txt"},
                      content="write a.txt\n第二行")) is False
...
def test_always_remembers_pattern_not_tool(make_perm, monkeypatch):
    ...
    assert asyncio.run(perm.check("run_command", {"command": "npm test"})) is True
    # 相同模式不再询问
    assert asyncio.run(perm.check("run_command", {"command": "npm test"})) is True
    # 其他模式仍会询问（第二次答案为 n）
    assert asyncio.run(perm.check("run_command", {"command": "npm run build"})) is False
...
def test_deny_blocks_even_with_approved_all(make_perm):
    ...
    assert asyncio.run(perm.check("run_command", {"command": "rm -rf /"})) is False
...
def test_user_allow_skips_asking(make_perm, monkeypatch):
    ...
    assert asyncio.run(perm.check("run_command", {"command": "git status"})) is True
...
def test_user_rules_override_defaults(make_perm, monkeypatch):
    ...
    assert asyncio.run(perm.check("read_file", {"path": "secret.env"})) is False
...
def test_unknown_action_degrades_to_ask(make_perm, monkeypatch):
    ...
    assert asyncio.run(perm.check("write_file", {"path": ".env"})) is False
...
def test_compound_command_any_ask_prompts(make_perm, monkeypatch):
    ...
    monkeypatch.setattr("builtins.input", lambda _: "n")
    assert asyncio.run(perm.check("run_command", {"command": "git status && rm -rf /"})) is False
...
def test_compound_command_all_allow_passes(make_perm, monkeypatch):
    ...
    refuse_input(monkeypatch)
    perm = make_perm(permissions={"run_command": {"git *": "allow"}})
    assert asyncio.run(perm.check("run_command", {"command": "git status && git log -1"})) is True
...
def test_pipe_segments_each_evaluated(make_perm, monkeypatch):
    ...
    monkeypatch.setattr("builtins.input", lambda _: "n")
    assert asyncio.run(perm.check("run_command", {"command": "git log | grep TODO"})) is False
    assert asyncio.run(perm.check("run_command", {"command": "git log"})) is True
...
def test_command_substitution_forces_ask(make_perm, monkeypatch):
    ...
    perm = make_perm(permissions={"run_command": {"git *": "allow"}})
    monkeypatch.setattr("builtins.input", lambda _: "n")
    assert asyncio.run(perm.check("run_command", {"command": "git log $(rm -rf /)"})) is False
    assert asyncio.run(perm.check("run_command", {"command": "git commit -m \"$(date)\""})) is False
    assert asyncio.run(perm.check("run_command", {"command": "echo `git push`"})) is False
    # 单引号内的 $( 不展开，不算替换
    assert asyncio.run(perm.check("run_command", {"command": "git commit -m '$(date)'"})) is True
...
def test_missing_config_uses_defaults(make_perm, monkeypatch):
    ...
    assert asyncio.run(perm.check("read_file", {"path": "a.txt"})) is True
...
def test_user_rule_matches_extra_root_path(make_perm, tmp_path, monkeypatch):
    ...
    assert asyncio.run(perm.check("write_file", {"path": f"../{extra.name}/src/a.py"})) is True
...
def test_ask_outside_access_renders_detail(tmp_path, monkeypatch, capture):
    ...
    assert asyncio.run(Permission().ask_outside_access("x.py", outside / "x.py")) == ("deny", None)
    assert cap.infos == []
...
def test_ask_reprompts_on_invalid_answer(make_perm, monkeypatch):
    ...
    assert asyncio.run(perm.check("run_command", {"command": "rm -rf build"})) is True
...
def test_ask_still_accepts_valid_answers(make_perm, monkeypatch):
    ...
    for answer, expected in (("y", True), ("n", False)):
        ...
        assert asyncio.run(perm.check("run_command", {"command": "rm -rf build"})) is expected
...
def test_non_interactive_ask_denied(make_perm, monkeypatch):
    ...
    assert asyncio.run(perm.check("write_file", {"path": "a.txt"})) is False
...
def test_check_paths_any_deny_rejects(make_perm, monkeypatch):
    ...
    refuse_input(monkeypatch)
    perm = make_perm()
    assert asyncio.run(perm.check_paths("apply_patch", ["src/a.py", ".git/config"])) is False
    assert asyncio.run(perm.check_paths("apply_patch", ["src/a.py", "x/.git/hooks/y.py"])) is False
...
def test_check_paths_any_ask_prompts(make_perm, monkeypatch):
    ...
    perm = make_perm()
    monkeypatch.setattr("builtins.input", lambda _: "y")
    assert asyncio.run(perm.check_paths("apply_patch", ["a.py", "b.py"])) is True
...
def test_check_paths_always_remembers_exact_patterns(make_perm, monkeypatch):
    ...
    monkeypatch.setattr("smithcode.permission.engine.confirmations_available", lambda: False)
    assert asyncio.run(perm.check_paths("apply_patch", ["a.py", "c.py"])) is False
...
def test_check_paths_approved_all_skips_ask(make_perm, monkeypatch):
    ...
    perm.approved_all = True
    assert asyncio.run(perm.check_paths("apply_patch", ["a.py", "b.py"])) is True
    assert asyncio.run(perm.check_paths("apply_patch", ["a.py", ".git/config"])) is False
...
def test_accept_edits_allows_edit_family_without_asking(make_perm, monkeypatch):
    ...
    assert asyncio.run(perm.check("edit_file", {"path": "src/a.py", "old_string": "x", "new_string": "y"})) is True
    assert asyncio.run(perm.check("write_file", {"path": "src/a.py"})) is True
    assert asyncio.run(perm.check_paths("apply_patch", ["a.py", "b.py"])) is True
...
def test_accept_edits_still_asks_run_command(make_perm, monkeypatch):
    ...
    perm.mode = "accept_edits"
    monkeypatch.setattr("builtins.input", lambda _: "n")
    assert asyncio.run(perm.check("run_command", {"command": "rm -rf build"})) is False
...
def test_accept_edits_respects_deny(make_perm, monkeypatch):
    ...
    perm = make_perm()
    perm.mode = "accept_edits"
    assert asyncio.run(perm.check("write_file", {"path": ".git/config"})) is False
...
def test_auto_mode_allows_all_ask_tools(make_perm, monkeypatch):
    ...
    perm.mode = "auto"
    assert asyncio.run(perm.check("write_file", {"path": "a.txt"})) is True
    assert asyncio.run(perm.check("run_command", {"command": "anything"})) is True
    assert asyncio.run(perm.check_paths("apply_patch", ["a.py"])) is True
...
def test_approved_all_alias_maps_to_auto(make_perm, monkeypatch):
    ...
    assert asyncio.run(perm.check("run_command", {"command": "ls"})) is True
...
def test_safe_command_skips_prompt(make_perm, monkeypatch):
    ...
    perm = make_perm()
    assert asyncio.run(perm.check("run_command", {"command": "git status"})) is True
    assert asyncio.run(perm.check("run_command", {"command": "git status && git diff"})) is True
...
def test_user_broad_ask_disables_safe_set(make_perm, monkeypatch):
    ...
    monkeypatch.setattr("builtins.input", lambda _: "n")
    perm = make_perm(permissions={"run_command": "ask"})
    assert asyncio.run(perm.check("run_command", {"command": "git status"})) is False
...
def test_user_specific_ask_keeps_safe_set_for_others(make_perm, monkeypatch):
    ...
    assert asyncio.run(perm.check("run_command", {"command": "git status"})) is True
    assert asyncio.run(perm.check("run_command", {"command": "git push origin"})) is False
...
def test_cd_with_git_prompts(make_perm, monkeypatch):
    ...
    monkeypatch.setattr("builtins.input", lambda _: "n")
    perm = make_perm()
    assert asyncio.run(perm.check("run_command", {"command": "cd sub && git status"})) is False
...
def test_cd_noop_with_git_allowed(make_perm, monkeypatch):
    ...
    refuse_input(monkeypatch)
    perm = make_perm()
    assert asyncio.run(perm.check("run_command", {"command": "cd . && git status"})) is True


def test_substitution_forces_prompt(make_perm, monkeypatch):
    monkeypatch.setattr("builtins.input", lambda _: "n")
    perm = make_perm()
    assert asyncio.run(perm.check("run_command", {"command": "git status && echo $(rm -rf /)"})) is False
...
def test_always_remembers_command_prefix(make_perm, monkeypatch):
    ...
    monkeypatch.setattr("builtins.input", lambda _: "a")
    perm = make_perm()
    assert asyncio.run(perm.check("run_command", {"command": "python -m pytest tests/a.py"})) is True
    assert ("run_command", ("python", "-m", "pytest"), "allow") in perm.session_rules

    refuse_input(monkeypatch)  # 后续不应再弹确认
    assert asyncio.run(perm.check("run_command", {"command": "python -m pytest tests/b.py"})) is True
    assert asyncio.run(perm.check("run_command", {"command": "python -m pytest -k foo"})) is True
    assert asyncio.run(perm.check("run_command", {"command": "CI=1 python -m pytest"})) is True
...
def test_prefix_memory_does_not_overreach(make_perm, monkeypatch):
    ...
    monkeypatch.setattr("builtins.input", lambda _: "a")
    perm = make_perm()
    asyncio.run(perm.check("run_command", {"command": "python -m pytest tests/a.py"}))

    monkeypatch.setattr("builtins.input", lambda _: "n")
    assert asyncio.run(perm.check("run_command", {"command": "python -m http.server"})) is False
    assert asyncio.run(perm.check("run_command", {"command": "python -c 'print(1)'"})) is False
    assert asyncio.run(perm.check("run_command", {"command": "python other.py"})) is False
...
def test_python_inline_code_memory_is_exact(make_perm, monkeypatch):
    ...
    monkeypatch.setattr("builtins.input", lambda _: "a")
    perm = make_perm()
    assert asyncio.run(perm.check("run_command", {"command": "python -c 'print(1)'"})) is True
    assert ("run_command", "python -c 'print(1)'", "allow") in perm.session_rules

    monkeypatch.setattr("builtins.input", lambda _: "n")
    assert asyncio.run(perm.check("run_command", {"command": "python -c 'print(2)'"})) is False
...
def test_unknown_command_memory_is_exact(make_perm, monkeypatch):
    ...
    monkeypatch.setattr("builtins.input", lambda _: "a")
    perm = make_perm()
    assert asyncio.run(perm.check("run_command", {"command": "mytool build --fast"})) is True
    assert ("run_command", "mytool build --fast", "allow") in perm.session_rules

    monkeypatch.setattr("builtins.input", lambda _: "n")
    assert asyncio.run(perm.check("run_command", {"command": "mytool build --slow"})) is False
