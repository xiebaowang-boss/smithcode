...
def test_ask_user_returns_answer(monkeypatch, capsys):
    """交互模式下打印问题并返回用户回答。"""
    monkeypatch.setattr("smithcode.tools.ask.read_user_input", lambda prompt="回答> ": "是的")
...
def test_ask_user_empty_answer(monkeypatch):
    """空白回答归一化为占位说明。"""
    monkeypatch.setattr("smithcode.tools.ask.read_user_input", lambda prompt="回答> ": "   ")
...
def test_ask_user_fail_closed_non_interactive(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        "smithcode.tools.ask.read_user_input",
...
def test_ask_user_uses_shared_read_user_input():
    ...
    from smithcode.tools import ask as ask_module
...
def test_ask_user_works_in_agent_loop(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("smithcode.tools.ask.confirmations_available", lambda: True)
    monkeypatch.setattr("smithcode.tools.ask.read_user_input", lambda prompt="回答> ": "继续")
